...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("thunderhotMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("thunderhotMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("thunderhotMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("thunderhot-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running thunderhotMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
