...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizethunderhotAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizethunderhotAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizethunderhotAmount = strAnonymizethunderhotAmount.remove(strAnonymizethunderhotAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizethunderhotAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizethunderhotAmount * COIN) nMaxToAnonymize = nAnonymizethunderhotAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizethunderhotAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizethunderhotAmount));
        strAnonymizethunderhotAmount = strAnonymizethunderhotAmount.remove(strAnonymizethunderhotAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizethunderhotAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizethunderhotAmount == 0) {
