...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a thunderhot address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no thunderhot: URI
...
{
    // Convert thunderhot:// to thunderhot:
    //
    //    Cannot handle this later, because thunderhot:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open thunderhot.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "thunderhot.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for thunderhot.lnk
...
{
    return GetAutostartDir() / "thunderhot.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a thunderhot.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=thunderhot\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the thunderhot app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add thunderhot app to startup item list
